...
def get_monthly_stats():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT COUNT(*) FROM placement_drive WHERE status IN ('approved', 'closed');"
    )
    stats["drives"] = cur.fetchone()[0]
    cur.execute(
...
def get_upcoming_deadline_reminders(within_days=3):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT u.email, s.name, d.job_title, c.company_name, d.application_deadline
        FROM application a
        JOIN student_profile s ON a.student_id = s.id
        JOIN users u ON s.user_id = u.id
        JOIN placement_drive d ON a.drive_id = d.id

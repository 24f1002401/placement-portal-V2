...
const AdminDashboard = {
  ...
  methods: {
    ...
    async runReminders() {
      ...
      this.message = data.success ? "Daily reminders job triggered." : (data.error || "Failed");
    },
    async runMonthly() {
      const { data } = await apiFetch("/api/jobs/run_monthly_report", { method: "POST", body: "{}" });
      this.message = data.success ? "Monthly report job triggered." : (data.error || "Failed");
    }
  },
  template: `
    <div>
...
const StudentDashboard = {
  ...
  methods: {
    ...
    async exportCsv() {
      ...
      this.message = data.message + (data.filename ? (" File: " + data.filename) : (" Task: " + data.task_id));
      if (data.filename) {
        this.message += " — download from /api/exports/" + data.filename + " (use token)";
      }
      alert(this.message);
    }
  },
